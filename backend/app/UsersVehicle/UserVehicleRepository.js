...
exports.getUserVehiclesByUserId = async (req, res) => {
    ...
    try {
        ...
        const [vehicles] = await db.query('SELECT * FROM user_vehicles WHERE user_id = ? ORDER BY created_at DESC', [userId]);
        res.status(200).json(vehicles);
    } catch (error) {
        console.error('Error fetching user vehicles by user ID:', error);
        res.status(500).json({ error: 'Failed to fetch user vehicles', details: error.message });
    }
};
